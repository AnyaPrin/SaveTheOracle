...
const gamestatstr="BAACBAACDFFEDIJEG..H";
const INIT_BOARD = [
    [2, 1, 1, 3],
    [2, 1, 1, 3],
    [4, 6, 6, 5],
...
let Board, Blks, AniIdx, Selected, PClr, clrAni, clrAniSTM, clr, isDrag, DSMP, DSBP;
...
function drawBlocks(mv) {
    for (let bid in Blks) {
        ...
        let [bx, by] = info.pos;
        let [bw, bh] = info.size;
        let x = bx * CELL + BDOFFX, y = by * CELL + BDOFFY;
...
function drawMessage() {
	...
	pctx.fillText("THE ORACLE ESCAPED!", w + pw, h + pw);

	// make pseudo3D-text
	pctx.fillStyle = "rgba(28,28,28,1)";
	pw=1; 
	pctx.fillText("THE ORACLE ESCAPED!", w + pw, h + pw); 

	pctx.fillStyle = CLR_TXT_COL; 
	pctx.fillText("THE ORACLE ESCAPED!", w, h);
...
function canMove(bid,mv) {
	...
	for (let x=0; x<bw; ++x) {
            let tx = nx + x, ty = ny + y;
            if (Board[ty][tx] !== 0 && Board[ty][tx] !== parseInt(bid))
		return false;
	}
    return true;
}
...
function move(bid,mv) {
    ...
    for (let y=0; y<bh; ++y)
	for (let x=0; x<bw; ++x)
	    Board[ny+y][nx+x] = parseInt(bid);

    if (bid == 1 && mv in OrclIdx) {
	AniIdx[1] = OrclIdx[mv];
	console.log(AniIdx[1]);
    }
