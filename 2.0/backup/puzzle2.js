...
function getMovableBlocksCount() {
    ...
    for (let i = 1; i < Blks.length; i++) {
        ...
        // 駒の左右にある空きスペースをチェック
        // 左側
        if (blk.pos[0] > 0 && blk.size[0] === 1 && emptyPos[0] === blk.pos[0] - 1
	    && emptyPos[1] === blk.pos[1]) {
            movableCount++;
        }
        // 右側
        if (blk.pos[0] + blk.size[0] < W && blk.size[0] === 1 && emptyPos[0] === blk.pos[0] + 1
	    && emptyPos[1] === blk.pos[1]) {
            movableCount++;
        }
        ...
        // 駒の上下にある空きスペースをチェック
        // 上側
        if (blk.pos[1] > 0 && blk.size[1] === 1 && emptyPos[1] === blk.pos[1] - 1 && emptyPos[0]
	    === blk.pos[0]) {
            movableCount++;
        }
        // 下側
        if (blk.pos[1] + blk.size[1] < H && blk.size[1] === 1 && emptyPos[1] === blk.pos[1] + 1 && emptyPos[0]
	    === blk.pos[0]) {
            movableCount++;
        }
        ...
        if (blk.size[0] === 2 && blk.size[1] === 2) {
             // 左に移動可能か
            if (blk.pos[0] > 0 && board[blk.pos[0] - 1][blk.pos[1]]
		=== 0 && board[blk.pos[0] - 1][blk.pos[1] + 1] === 0) {
                movableCount++;
            }
            // 右に移動可能か
            if (blk.pos[0] + blk.size[0] < W && board[blk.pos[0] + blk.size[0]][blk.pos[1]]
		=== 0 && board[blk.pos[0] + blk.size[0]][blk.pos[1] + 1] === 0) {
                movableCount++;
            }
             // 上に移動可能か
            if (blk.pos[1] > 0 && board[blk.pos[0]][blk.pos[1] - 1]
		=== 0 && board[blk.pos[0] + 1][blk.pos[1] - 1] === 0) {
                movableCount++;
            }
            // 下に移動可能か
            if (blk.pos[1] + blk.size[1] < H && board[blk.pos[0]][blk.pos[1] + blk.size[1]]
		=== 0 && board[blk.pos[0] + 1][blk.pos[1] + blk.size[1]] === 0) {
                movableCount++;
            }
        }
    }
...
function drawAll() {
    ...
    Blks.forEach(blk => {
        if (blk.type !== "empty") {
            ...
            ctx.fillRect(BDOFFX + blk.pos[0]*CELL,
			 BDOFFY + blk.pos[1]*CELL, blk.size[0]*CELL - BLK_BRDR, blk.size[1] * CELL - BLK_BRDR);
