...
const ORA_COL = "#C8C8B4";
const TXT_DARK = "#002828";
const TXT_LIGHT = "#FFFFFF";
const CLR_TXT_COL = TXT_LIGHT;

const SPRITE_PATH = "img/spritesheet.png" // sprites sheet
const SPRITE_MAP = { 
    'oD':    { x:    0, y: 0, w: 200, h: 200 },  // oracle
    'oR':    { x:  200, y: 0, w: 200, h: 200 },  
    'oU':    { x:  400, y: 0, w: 200, h: 200 },
    'oL':    { x:  600, y: 0, w: 200, h: 200 },
    'b2':    { x:    0, y: 200, w: 100, h: 200 }, // lion 
    'b3':    { x:  100, y: 200, w: 100, h: 200 }, // 
    'b4':    { x:  200, y: 200, w: 100, h: 200 }, // blue     
    'b5':    { x:  300, y: 200, w: 100, h: 200 }, // red
    'b6':    { x:  400, y: 200, w: 200, h: 100 }, //
    'b7':    { x:  600, y: 300, w: 100, h: 100 },
    'b8':    { x:  600, y: 200, w: 100, h: 100 },    
    'b9':    { x:  400, y: 300, w: 100, h: 100 }, // red    
    'b10':   { x:  500, y: 300, w: 100, h: 100 },
    'retry':    { x:  700, y: 200, w: 100, h: 100 },
    'hint':     { x:  700, y: 300, w: 100, h: 100 },  // for extension in future
    'castle':   { x:    0, y: 400, w: 600, h: 700 },    // background image of grid    
...
async function loadAllResources() {
    const imgSheet = await ldImgSheet(SPRITE_PATH);
    const files = ["oU", "oD", "oL", "oR"];
    IMGSs = {};
    OIMG_IDX = {};
...
function drawOracle(offset_x, offset_y){
    Object.keys(OIMG_IDX).find(k => OIMG_IDX[k] === AIdx[1]);
    let info = blks[1];
    let [bx, by] = info.pos;
    let [bw, bh] = info.size;
    let x = bx * CELL + offset_x;
    let y = by * CELL + offset_y;
    let skey = `b1`;    
    let map = SPRITE_MAP[skey];    
    let img = IMGs[skey];    
    if ( MrflshAniAct && MrflshPh == 1) {
	let elapsed = performance.now() - MrflshPhST;
	let frames = 4;
	let duration = MRFLSH_ROT_DUR / num_frames;
	let idx = Math.floor((elapsed / duration) % frames);
	let keys = ["oD", "oR", "oU", "oL"];
	aniIdx[1] = OIMG_IDX[keys[idx]] || 0
	ctx.drawImage(img, map.x, map.y, map.w, map.h, x, y, bw*CELL, bh*CELL);	;
    } else {
	ctx.strokeStyle = selected == 1 ? SBLK_COL : "rgba(0,0,0,0)";
	ctx.lineWidth = BLK_BRDR;
	ctx.strokeRect(x, y, bw*CELL, bh*CELL);
    }
	ctx.fillStyle = ORA_COL;
...
function drawBlocks(offset_x, offset_y) {
	...
	let y = by * CELL + offset_y;
	    ...
	    ctx.fillStyle = (selected == bid ? SBLK_COL : BLK_COL);
	    ctx.fillRect(x, y, bw*CELL, bh*CELL);
	    ctx.strokeStyle = FLR_COL;
	    ctx.strokeRect(x, y, bw*CELL, bh*CELL);
        }
...
function drawAll() {
    ...
    ctx.drawImage(imgSheet, 0, 0, SCRN_W, SCRN_H);
