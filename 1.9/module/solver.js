import { SolutionDisplay } from './solution.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const topContainer = document.querySelector('.ui-panel');
    ...
    function startSearch(selectedAlgorithm) {
        ...
        if (useLocalVisited) {
            ...
            if (dataStore.set.has(normalizedInitialStateBigInt)) {
                ...
                statusDiv.textContent = '情報: 探索開始盤面が保存済みデータに含まれていたため保存データは利用されません。';
            } else {
    ...
    fetch(ASSET_PATHS.DATA_FILES.optimalPath)
        ...
        .then(data => {
            ...
            pruningDataStatus.textContent = `(${optimalPathData.rawSet.size.toLocaleString()})`;
        })
        .catch(error => {
