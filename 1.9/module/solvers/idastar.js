...
export class IDAStarSolver {
    ...
    _heuristic(stateBigInt) {
        ...
        const blkPos = normalizedString.indexOf(COMMON.GOAL_BLK);
        if (blkPos === -1) return Infinity;

        const currentX = blkPos % COMMON.WIDTH;
        const currentY = Math.floor(blkPos / COMMON.WIDTH);
        const goalX = 1;
        const goalY = 3;
