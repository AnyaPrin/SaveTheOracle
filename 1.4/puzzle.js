...
const ULRECT = [0, SCRN_H-WALL*1.8, CELL*2, CELL*2];
...
const DRAG_THLD = CELL * 0.1; // mouse drag sensibility 0<fast<->slow >1
...
const onMouseMove = (e) => {
    ...
    if (mv) {
	    ...
	    DSBP = [...Blks[Selected].pos];  //  Selected block position
        } else {
	    DSMP = [x, y];                   //
...
const onMouseMove = (e) => {
    ...
    if (0 <= gx && gx < W && 0 <= gy && gy < H) {     // 1. マウスオーバーした駒を特定
        let hovered_bid = Brd[gy][gx];
	console.log(`hovered_bid = ${Brd[gy][gx]}`);
        if (hovered_bid !== 0) {
            if (Selected !== hovered_bid) {             // 前の駒と違う場合のみ処理
                Selected = hovered_bid;
                currentMoves = {                 // 2. 移動可能性を計算
                    up: canMove(Selected, "up"),
                    down: canMove(Selected, "down"),
                    left: canMove(Selected, "left"),
                    right: canMove(Selected, "right")
                };
	    ...
	    Selected = null;             // 空のマスにいる場合は選択を解除	    
            currentMoves = {};
	}
    }
    // ドラッグ中の描画ロジックはここに残す
    if (!isDrag || !Selected) return;
    // ... （既存の onMouseMove のドラッグ中の描画ロジック）
};
...
const onMouseDown = (e) => {    // マウスダウン時は、ドラッグ開始のフラグと座標だけを設定
    if (Selected) {
        isDrag = true;
        let rect = puzzleCanvas.getBoundingClientRect();
        DSMP = [e.clientX - rect.left, e.clientY - rect.top];
        DSBP = [...Blks[Selected].pos];	
    }
};
*/
const onMouseUp = (e) => {
    console.log(`MOSUE UP! isDrag ${isDrag} `);
    ...
    if (Math.abs(dy) > DRAG_THLD && Math.abs(dy) > Math.abs(dx)) mv = dy > 0 ? "down" : "up";
    console.log(`mv:${mv}`);
    if (mv && currentMoves[mv]) {     // 3. 事前に計算した移動可能性をチェック
	console.log("direction:",mv);
    }
    move(Selected, mv);    
    isDrag = false;
    Selected = null; // マウスアップ時に選択を解除する
    currentMoves = {};
};
